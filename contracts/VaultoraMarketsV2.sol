...
contract VaultoraMarketsV2 {
    ...
    // Use initializer instead of constructor for proxy compatibility
    ...
    function createMarket(
        ...
        m.status = 0;
        for (uint256 i = 0; i < options.length; i++) m.options.push(options[i]);
    ...
    function buy(uint256 marketId, uint8 outcome, uint256 amount) external {
        ...
        uint256 tokens = net; // 1:1 fair ratio
    ...
    function toggleToken(uint8 idx, bool enabled) external onlyOwner { paymentTokens[idx].enabled = enabled; }
    ...
    function getPosition(uint256 marketId, address user) external view returns (
        uint256[] memory holdings, uint256[] memory balances
    ) {
        Market storage m = markets[marketId];
