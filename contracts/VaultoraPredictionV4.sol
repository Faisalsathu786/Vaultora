...
contract VaultoraPredictionV4 is
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _usdc, address _eurc) public initializer {
        require(_usdc != address(0), "V4: zero USDC");
        require(_eurc != address(0), "V4: zero EURC");
    ...
    function setEurcRate(uint256 _eurcRate) external onlyOwner {
        require(_eurcRate > 0, "V4: zero rate");
    ...
    function addPaymentToken(address token) external onlyOwner {
        require(token != address(0), "V4: zero address");
